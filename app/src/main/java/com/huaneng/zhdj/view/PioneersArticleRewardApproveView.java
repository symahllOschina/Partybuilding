 ...
 * 时代先锋：作品评论、打赏、点赞View
 */
public class PioneersArticleRewardApproveView extends LinearLayout {
    ...
    public void init(BaseActivity ctx, String articleId) {
        ...
        approveView.init(ctx, articleId, new PioneersWorksApprove());
    }

    public PioneersArticleRewardApproveView(Context context) {
        super(context);
    }

    public PioneersArticleRewardApproveView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public PioneersArticleRewardApproveView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public PioneersArticleRewardApproveView(Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void publishComment(String content) {
        ...
        HTTP.service.post("comment/save", MapParam.me().p("works_id", articleId).p("to_user_id", "0").p("content", content).build())
    ...
    private void reward() {
        ...
        HTTP.service.post("integral/save", MapParam.me().p("works_id", articleId).p("num", (num + 1)).build())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Subscriber(ctx){
